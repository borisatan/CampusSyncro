...
export const SpendingTrendChart = React.memo(
  ({
    ...
    // SharedValues for worklet-safe access — plain JS closure variables can
    // be stale in Reanimated worklets when the component re-renders with new
    // props (e.g. timeFrame changing), causing wrong per-data-point granularity.
    const stepsShared = useSharedValue(INTERPOLATION_STEPS);
    const smoothLastActualShared = useSharedValue(smoothLastActualIndex);
    // Keep in sync whenever the derived values change
    React.useEffect(() => {
      stepsShared.value = INTERPOLATION_STEPS;
    }, [timeFrame]);
    React.useEffect(() => {
      smoothLastActualShared.value = smoothLastActualIndex;
    ...
    const handleTooltipUpdate = (rawIndex: number) => {
      ...
      // Map back to the actual (non-interpolated) data point
    ...
    useAnimatedReaction(
      ...
      (current) => {
        ...
        const lastActual = smoothLastActualShared.value;

        // Update clamped positions for tooltip rendering
        if (rawIndex > lastActual && lastActual >= 0) {
        ...
        // Map to original data-point index to debounce JS bridge calls —
        // only cross the bridge when the user moves to a new data point.
        // Use SharedValues (not closure captures) so the worklet always sees
        // the correct INTERPOLATION_STEPS for the current timeFrame.
        const clampedRaw =
          rawIndex > lastActual && lastActual >= 0 ? lastActual : rawIndex;
        const originalIndex = Math.round(clampedRaw / stepsShared.value);
