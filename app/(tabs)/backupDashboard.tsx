    ...
    <SafeAreaView className="flex-1 bg-slate-950">
      <ScrollView className="flex-1 bg-slate-950">
        <View className="p-2">
          ...
          {/* Recent Transactions */}
          <View className="bg-slate-900 rounded-2xl p-4 border border-slate-800 mb-6">
            <Text className="text-white text-base font-semibold mb-4">Recent Transactions</Text>
            <View className="gap-3">
              {recentTransactions.map((transaction) => (
                <View key={transaction.id} className="flex-row items-center justify-between mb-3">
                  <View className="flex-row items-center gap-3">
                    <View className={`w-10 h-10 rounded-full items-center justify-center ${
                      transaction.amount > 0 ? 'bg-emerald-500/20' : 'bg-slate-800'
                    }`}>
                      <DollarSign
                        color={transaction.amount > 0 ? '#34d399' : '#94a3b8'}
                        size={20}
                      />
                    </View>
                    <View>
                      <Text className="text-white text-sm">{transaction.name}</Text>
                      <Text className="text-slate-500 text-xs mt-0.5">{transaction.date}</Text>
                    </View>
                  </View>
                  <Text className={transaction.amount > 0 ? 'text-emerald-400 text-sm' : 'text-white text-sm'}>
